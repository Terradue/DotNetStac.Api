...
namespace Stac.Api.Tests.AppTests
{
    ...
    public class SortApiTests : AppTestBase
    {

        private readonly JsonSerializerSettings _settings;

        public SortApiTests(StacApiAppFixture fixture, ITestOutputHelper outputHelper) : base(fixture, outputHelper)
        {
        ...
        public async Task PostItemSearchAsync(StacApiApplication application)
        {
            ...
            body.AdditionalProperties.Add("sortby", new Sortby(){
                    new SortByItem(){
                        Field = "id",
                        Direction = SortByItemDirection.Desc
                    }
            });

            StacFeatureCollection result = await filterClient.PostItemSearchAsync(
                body
            );
            ...
            foreach (var feature in result.Features)
            {
                if (previousId != null)
                {
                    Assert.True(string.CompareOrdinal(previousId, feature.Id) < 0);
                }
